...
class bfs{
 ...
 bfs(int extent_x, int extent_y): world_extents_x(extent_x), world_extents_y(extent_y){
	...
	void compute(){
		...
		open_list.push(Node(start_x, start_y, -1, -1)); 
		...
		//	std::cout<<"open_list "<< open_list.size() << " "; 
			...
			for(auto i=0; i<movements.size(); i++){
				...
				if(world_obstacles[new_x][new_y] ==0 && visited[new_x][new_y] == 0){
					...
					Node n = Node(new_x, new_y, element.x, element.y);
					open_list.push(n);
	...
	//	double cost;
    	...
    	// of the structure
    	Node(int x_, int y_, int prev_x_, int prev_y_)
        	: x(x_), y(y_), prev_x(prev_x_), prev_y(prev_y_) {}
	};
 /*
...
*/
	...
	vector<move> movements = { move(1,0), move(0,1), move(-1,0), move(0,-1),
...
BOOST_PYTHON_MODULE(pp_03) {
	...
	class_ <bfs>( "bfs", 
        ...
        .def("fill_obstacles", &bfs::fill_obstacles)
        .def("fill_start_goal", &bfs::fill_start_goal)
		.def("return_visited", &bfs::return_visited)
		.def("return_path", &bfs::return_path)
		.def("compute", &bfs::compute)
		.def("get_path_dim1", &bfs::get_path_dim1)
		.def("get_path_dim2", &bfs::get_path_dim2)
    ;
