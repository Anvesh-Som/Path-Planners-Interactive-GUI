...
class Astar{
	...
	Astar(int extent_x, int extent_y): world_extents_x(extent_x), world_extents_y(extent_y){
	...
	void compute(){
		...
		open_list.push(Node(start_x, start_y, -1, -1, 0.0, distance_n_g(start_x, start_y, goal_x, goal_y))); 
		...
		//	std::cout<<"open_list "<< open_list.size() << " "; 
			...
			for(auto i=0; i<movements.size(); i++){
                ...
                double new_total_cost = new_cost + distance_n_g(element.x, element.y, goal_x, goal_y);
				...
				if(world_obstacles[new_x][new_y] ==0 && visited[new_x][new_y] == 0){
					...
					Node n = Node(new_x, new_y, element.x, element.y , new_cost, new_total_cost);
					open_list.push(n);
    ...
    double distance_n_g(int s_x, int s_y, int g_x, int g_y){ 
        return sqrt(pow((g_x-s_x),2)+ pow((g_y-s_y),2));
    }
	...
	struct Node {
    	...
    	// of the structure
    	Node(int x_, int y_, int prev_x_, int prev_y_, double cost_, double total_cost_)
        	: x(x_), y(y_), prev_x(prev_x_), prev_y(prev_y_), cost(cost_), total_cost(total_cost_) {}
	};
	...
	struct CompareCost {
    	...
    	{
        	return n1.total_cost > n2.total_cost; //Implementation of in_heap (min at top)
   	 	}
	};
...
BOOST_PYTHON_MODULE(pp_02) {
	...
	class_<Astar>("Astar", 
        ...
        .def("fill_obstacles", &Astar::fill_obstacles)
        .def("fill_start_goal", &Astar::fill_start_goal)
		.def("return_visited", &Astar::return_visited)
		.def("return_path", &Astar::return_path)
		.def("compute", &Astar::compute)
		.def("get_path_dim1", &Astar::get_path_dim1)
		.def("get_path_dim2", &Astar::get_path_dim2)
    ;
